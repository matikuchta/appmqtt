...
function update(p = "") {
  ...
  if (p === "") {
    ...
    if (nazwisko.value.trim() !== "")
      updatedData.nazwisko = nazwisko.value.trim();
    if (stanowisko.value.trim() !== "")
      updatedData.stanowisko = stanowisko.value.trim();
    if (dataUrodzenia.value !== "")
      updatedData.data_urodzenia = dataUrodzenia.value;
    if (dataZatrudnienia.value !== "")
      updatedData.data_zatrudnienia = dataZatrudnienia.value;
  } else {
...
function getallpersons(edit = "") {
  ...
  res += "</tr>";
  console.log("endpoint persons: ", path + "/persons/");
  fetch(path + "/persons/", {
    headers: {
      "Content-Type": "application/json",
      Authorization: "Bearer " + token,
    ...
    .then((response) => {
      if (response.status === 403) {
        console.log("ThAccess deniedrow: ");
      ...
      console.log("getallpersons res ", response.body);

      const resJson = response.json();
      console.log("getallpersons json ", resJson);
    ...
    .then((response) => {
      ...
      for (const person of response) {
        ...
        if (edit != person["pesel"]) {
          ...
          res +=
            "<td>" + person["data_zatrudnienia"].substring(0, 10) + "</td>";
          res +=
            "<td><button onclick='deleteperson(" +
            person.pesel +
            ")'>X</button></td>";
          res +=
            "<td><button onclick='getallpersons(" +
            person.pesel +
            ")'>/</button></td>";
        } else {
          res += "<th>" + ++i + "</th>";
          ...
          res +=
            ...
            person["data_urodzenia"].substring(0, 10) +
            "' id='udob'></td>";
          res +=
            "<td><input type='date' value='" +
            person["data_zatrudnienia"].substring(0, 10) +
            "' id='uhired'></td>";
...
refreshAccessToken().then(getallpersons());
    ...
    .then(({ status, body }) => {
      if (status === 200) {
        ...
        loginresponse.innerHTML =
          "Login failed: " + (body.detail || "Unknown error");
      }
    })
    .catch((error) => {
          ...
          .then(({ status, body }) => {
            if (status === 200) {
              ...
              signupresponse.innerHTML +=
                "<br>Login failed: " + (body.detail || "Unknown error");
            }
          });
      } else {
        signupresponse.innerHTML =
          "Sign up failed: " + (body.detail || "Unknown error");
      }
    })
    .catch((error) => {
      console.error("Login error:", error);
  ...
  fetch(path + "/auth/users/reset_password/", {
    ...
    .then((response) => {
      if (response.status === 204) {
        resetResponse.innerText =
          "E-mail do resetu hasła został wysłany, sprawdź swoją skrzynkę.";
      } else {
        return response.json().then((data) => {
          resetResponse.innerText = data.email
            ? data.email.join(" ")
            : "Coś poszło nie tak.";
        });
      }
